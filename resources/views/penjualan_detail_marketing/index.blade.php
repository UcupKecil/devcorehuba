...
<tbody></tbody>
  ...
  <form class="form form-horizontal form-penjualan" method="post" action="transaksi_marketing/simpan">
      {{ csrf_field() }}
  <div class="col-md-3">
...
@include('penjualan_detail_marketing.produk')
@include('penjualan_detail_marketing.member')
@endsection
...
$(function(){
  ...
  table = $('.tabel-penjualan').DataTable({
     ...
     "processing" : true,
     "ajax" : {
       "url" : "{{ route('transaksi_marketing.data', $idpenjualan) }}",
       "type" : "GET"
     }
  }).on('draw.dt', function(){
    loadForm($('#diskon').val());
...
function addItem(){
  $.ajax({
    url : "{{ route('transaksi_marketing.store') }}",
    type : "POST",
...
function loadForm(diskon=0, diterima=0){
  ...
  $.ajax({
       url : "transaksi_marketing/loadform/"+diskon+"/"+$('#total').val()+"/"+diterima,
       type : "GET",
       dataType : 'JSON',
       success : function(data){
