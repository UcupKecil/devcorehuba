...
<tbody></tbody>
  ...
  <form class="form form-horizontal form-penjualan" method="post" action="transaksi_reseller/simpan">
      {{ csrf_field() }}
  <div class="col-md-3">
      ...
      <div class="form-group">
        <label for="sales" class="col-md-4 control-label">Pilih Sales</label>
        <div class="col-md-8">
          <div class="input-group">
            <input id="sales" type="text" class="form-control" name="sales" placeholder="Pilih jika sudah menjadi sales">
            <span class="input-group-btn">
              <button onclick="showSales()" type="button" class="btn btn-info">Add...</button>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="form-group">
        <label for="alamat_kirim" class="col-md-4 control-label">Alamat Kirim</label>
        <div class="col-md-8">
...
@include('penjualan_detail_reseller.produk')
@include('penjualan_detail_reseller.member')
@include('penjualan_detail_reseller.sales')
@endsection
...
$(function(){
  ...
  table = $('.tabel-penjualan').DataTable({
     ...
     "processing" : true,
     "ajax" : {
       "url" : "{{ route('transaksi_reseller.data', $idpenjualan) }}",
       "type" : "GET"
     }
  }).on('draw.dt', function(){
    loadForm($('#diskon').val());
...
function addItem(){
  $.ajax({
    url : "{{ route('transaksi_reseller.store') }}",
    type : "POST",
...
function selectSales(kode,nama,alamat){
...
function loadForm(diskon=0, diterima=0){
  ...
  $.ajax({
       url : "transaksi_reseller/loadform/"+diskon+"/"+$('#total').val()+"/"+diterima,
       type : "GET",
       dataType : 'JSON',
       success : function(data){
